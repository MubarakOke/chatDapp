...
var AccountController = {
  ...
  setSmartAccountDeployed(isDeployed) {
    state2.smartAccountDeployed = isDeployed;
  },
  resetAccount() {
    ...
    state2.smartAccountDeployed = void 0;
  }
};
...
var OptionsController = {
  ...
  setProjectId(projectId) {
    state4.projectId = projectId;
  },
  setAllWallets(allWallets) {
    state4.allWallets = allWallets;
  },
  setIncludeWalletIds(includeWalletIds) {
    state4.includeWalletIds = includeWalletIds;
  },
  setExcludeWalletIds(excludeWalletIds) {
    state4.excludeWalletIds = excludeWalletIds;
  },
  setFeaturedWalletIds(featuredWalletIds) {
    state4.featuredWalletIds = featuredWalletIds;
  },
  setTokens(tokens) {
    state4.tokens = tokens;
  },
  setTermsConditionsUrl(termsConditionsUrl) {
    state4.termsConditionsUrl = termsConditionsUrl;
  },
  setPrivacyPolicyUrl(privacyPolicyUrl) {
    state4.privacyPolicyUrl = privacyPolicyUrl;
  },
  setCustomWallets(customWallets) {
    state4.customWallets = customWallets;
  },
  setIsSiweEnabled(isSiweEnabled) {
    state4.isSiweEnabled = isSiweEnabled;
  },
  setEnableAnalytics(enableAnalytics) {
    state4.enableAnalytics = enableAnalytics;
  },
  setSdkVersion(sdkVersion) {
    state4.sdkVersion = sdkVersion;
  },
  setMetadata(metadata) {
    state4.metadata = metadata;
  },
  setOnrampEnabled(enableOnramp) {
    state4.enableOnramp = enableOnramp;
  },
  setWalletFeaturesEnabled(enableWalletFeatures) {
    state4.enableWalletFeatures = enableWalletFeatures;
  }
};
...
var ThemeController = {
  ...
  setThemeVariables(themeVariables) {
    state5.themeVariables = { ...state5.themeVariables, ...themeVariables };
    try {
      const emailConnector = ConnectorController.getEmailConnector();
      if (emailConnector) {
        emailConnector.provider.syncTheme({
...
var ConnectorController = {
  ...
  setConnectors(connectors) {
    state6.connectors = connectors.map((c) => ref(c));
  },
  addConnector(connector) {
    var _a5, _b3;
    state6.connectors.push(ref(connector));
  ...
  getEmailConnector() {
    return state6.connectors.find((c) => c.type === "EMAIL");
  },
  getAnnouncedConnectorRdns() {
    return state6.connectors.filter((c) => c.type === "ANNOUNCED").map((c) => {
      var _a5;
      return (_a5 = c.info) == null ? void 0 : _a5.rdns;
    });
  },
  getConnectors() {
    return state6.connectors;
  }
};
...
var NetworkController = {
  ...
  setCaipNetwork(caipNetwork) {
    state9.caipNetwork = caipNetwork;
  ...
  setDefaultCaipNetwork(caipNetwork) {
    state9.caipNetwork = caipNetwork;
    PublicStateController.set({ selectedNetworkId: caipNetwork == null ? void 0 : caipNetwork.id });
    state9.isDefaultCaipNetwork = true;
  },
  setRequestedCaipNetworks(requestedNetworks) {
    state9.requestedCaipNetworks = requestedNetworks;
  },
  setAllowUnsupportedChain(allowUnsupportedChain) {
    state9.allowUnsupportedChain = allowUnsupportedChain;
  },
  getRequestedCaipNetworks() {
    const { approvedCaipNetworkIds, requestedCaipNetworks } = state9;
  ...
  async getApprovedCaipNetworksData() {
    ...
    state9.supportsAllNetworks = data.supportsAllNetworks;
    state9.approvedCaipNetworkIds = data.approvedCaipNetworkIds;
  },
  async switchActiveNetwork(network) {
    await this._getClient().switchCaipNetwork(network);
    state9.caipNetwork = network;
    if (network) {
  ...
  checkIfSupportedNetwork() {
    var _a5;
    state9.isUnsupportedChain = !((_a5 = state9.requestedCaipNetworks) == null ? void 0 : _a5.some((network) => {
      var _a6;
      return network.id === ((_a6 = state9.caipNetwork) == null ? void 0 : _a6.id);
    }));
    if (state9.isUnsupportedChain) {
      this.showUnsupportedChainUI();
    }
  },
  resetNetwork() {
    if (!state9.isDefaultCaipNetwork) {
      state9.caipNetwork = void 0;
    }
    state9.approvedCaipNetworkIds = void 0;
    state9.supportsAllNetworks = true;
  },
...
var ApiController = {
  ...
  async fetchRecommendedWallets() {
    ...
    state10.recommended = data;
    state10.count = count ?? 0;
  },
  async fetchWallets({ page }) {
    const { includeWalletIds, excludeWalletIds, featuredWalletIds } = OptionsController.state;
    const exclude = [
      ...state10.recommended.map(({ id }) => id),
      ...excludeWalletIds ?? [],
      ...featuredWalletIds ?? []
    ].filter(Boolean);
    const { data, count } = await api2.get({
    ...
    state10.wallets = [...state10.wallets, ...data];
    state10.count = count > state10.count ? count : state10.count;
    state10.page = page;
  },
  ...
  prefetch() {
    ...
    state10.prefetchPromise = Promise.race([Promise.allSettled(promises), CoreHelperUtil.wait(3e3)]);
  },
...
var RouterController = {
  ...
  replace(view, data) {
    if (state11.history.length > 1 && state11.history.at(-1) !== view) {
      state11.view = view;
...
var state13 = proxy(defaultState);
var OnRampController = {
  ...
  setSelectedProvider(provider) {
    state13.selectedProvider = provider;
  },
  setPurchaseCurrency(currency) {
    state13.purchaseCurrency = currency;
  },
  setPaymentCurrency(currency) {
    state13.paymentCurrency = currency;
  },
  ...
  async getAvailableCurrencies() {
    ...
    state13.purchaseCurrencies = options.purchaseCurrencies;
    state13.paymentCurrencies = options.paymentCurrencies;
    state13.paymentCurrency = options.paymentCurrencies[0] || USD_CURRENCY_DEFAULT;
    state13.purchaseCurrency = options.purchaseCurrencies[0] || USDC_CURRENCY_DEFAULT;
    await ApiController.fetchCurrencyImages(options.paymentCurrencies.map((currency) => currency.id));
    await ApiController.fetchTokenImages(options.purchaseCurrencies.map((currency) => currency.symbol));
  },
  async getQuote() {
    var _a5, _b3;
    state13.quotesLoading = true;
    try {
      const quote = await BlockchainApiController.getOnrampQuote({
        purchaseCurrency: state13.purchaseCurrency,
        paymentCurrency: state13.paymentCurrency,
        amount: ((_a5 = state13.paymentAmount) == null ? void 0 : _a5.toString()) || "0",
        network: (_b3 = state13.purchaseCurrency) == null ? void 0 : _b3.symbol
      });
      state13.quotesLoading = false;
      state13.purchaseAmount = Number(quote.purchaseAmount.amount);
      return quote;
    } catch (error) {
      state13.error = error.message;
      state13.quotesLoading = false;
      return null;
    } finally {
      state13.quotesLoading = false;
  ...
  resetState() {
    state13.providers = ONRAMP_PROVIDERS;
    state13.selectedProvider = null;
    state13.error = null;
    state13.purchaseCurrency = USDC_CURRENCY_DEFAULT;
    state13.paymentCurrency = USD_CURRENCY_DEFAULT;
    state13.purchaseCurrencies = [USDC_CURRENCY_DEFAULT];
    state13.paymentCurrencies = [];
    state13.paymentAmount = void 0;
    state13.purchaseAmount = void 0;
    state13.quotesLoading = false;
  }
};
...
var SnackController = {
  ...
  showSuccess(message) {
    ...
    state14.variant = "success";
    state14.open = true;
  },
  showError(message) {
    const errorMessage = CoreHelperUtil.parseError(message);
    state14.message = errorMessage;
    state14.variant = "error";
    state14.open = true;
  },
  hide() {
    state14.open = false;
...
WuiGrid.styles = [resetStyles, styles_default59];
__decorate59([
  property()
...
WuiSeparator.styles = [resetStyles, styles_default60];
__decorate60([
  property()
], WuiSeparator.prototype, "text", void 0);
WuiSeparator = __decorate60([
...
lit-html/development/directives/ref.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-OQ6TX2IK.js.map
